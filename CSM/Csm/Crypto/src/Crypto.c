...
STATIC boolean insertJob(Crypto_JobType* job){
...
//invoke higher priority job to process which is the front process but need to check if it cancelled or no jobs in queue
 if(jobQueue.arr[jobQueue.front].jobPriority != -1 && jobQueue.size != 0 && main_counter <= QUEUE_MAX_SIZE )
 {
